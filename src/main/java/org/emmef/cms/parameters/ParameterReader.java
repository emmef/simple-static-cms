...
public class ParameterReader {
	...
	public ParameterResults read(@NonNull String[] commandLine, @NonNull Parameter configParameter, @NonNull String relativeConfigPath) {
		return read(commandLine, Environment.SYSTEM_PROPERTIES, Environment.SYSTEM_ENVIRONMENT, configParameter, relativeConfigPath);
	}

	public ParameterResults read(@NonNull String[] commandLine, @NonNull Environment systemProperties, @NonNull Environment environmentVariables, @NonNull Parameter configParameter, @NonNull String relativeConfigPath) {
		ParameterResults resultsWithoutConfig = createResults();

		readFromAllButConfigurationFile(commandLine, systemProperties, environmentVariables, resultsWithoutConfig);

		String configRoot = resultsWithoutConfig.getValue(configParameter);
		Properties configurationProperties = getConfigurationProperties(configRoot, relativeConfigPath);

		if (configurationProperties == null) {
			substituteDefaults(resultsWithoutConfig);
			verifyMandatory(resultsWithoutConfig);
			return resultsWithoutConfig;
		}

		ParameterResults results = createResults();

		readFromAllButConfigurationFile(commandLine, systemProperties, environmentVariables, results);
		readFromGeneric(results, Environment.wrappedProperties(configurationProperties), NameAndDisplay.CONFIGURATION_PROPERTY);
	...
	private void readFromAllButConfigurationFile(@NonNull String[] commandLine, @NonNull Environment systemProperties, @NonNull Environment environmentVariables, ParameterResults resultsWithoutConfig) {
		readFromCommandLine(resultsWithoutConfig, commandLine);
		readFromSystemProperties(resultsWithoutConfig, systemProperties);
		readFromEnvironment(resultsWithoutConfig, environmentVariables);
	}

	private static Properties getConfigurationProperties(@NonNull String configRoot, @NonNull String relativeConfigPath) {
		if (configRoot != null && !configRoot.isBlank()) {
			Path configFilePath = Path.of(configRoot).resolve(relativeConfigPath);

			try {
				Properties p = new Properties();
				p.load(Files.newInputStream(configFilePath));
	...
	interface NameAndDisplay {
		...
		NameAndDisplay CONFIGURATION_PROPERTY = new NameAndDisplay() {
			@Override
			public String getName(Parameter parameter) {
				return parameter.getName();
			}
			...
			@Override
			public String getDisplay(Parameter parameter) {
				return "Configuration property " + parameter.getNameKey();
			}
		};
	}
}
